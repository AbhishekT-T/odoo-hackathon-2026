...
const Vehicle = {
  ...
  create: async (data) => {
    const { registration_number, name, type, max_load_capacity, odometer, acquisition_cost, status, region } = data;
    const result = await db.query(
      `INSERT INTO vehicles (registration_number, name, type, max_load_capacity, odometer, acquisition_cost, status, region) 
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8) 
       RETURNING *`,
      [registration_number, name, type, max_load_capacity, odometer || 0, acquisition_cost || 0, status || 'Available', region || 'National']
    );
    return result.rows[0];
  },
  ...
  update: async (id, data) => {
    ...
    const result = await db.query(
      `UPDATE vehicles 
       SET registration_number = $1, name = $2, type = $3, max_load_capacity = $4, odometer = $5, acquisition_cost = $6, status = $7, region = $8 
       WHERE id = $9 
       RETURNING *`,
      [registration_number, name, type, max_load_capacity, odometer, acquisition_cost, status, region || 'National', id]
    );
    return result.rows[0];
  },
